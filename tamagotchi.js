...
const gameLoop = () => {
  if (partner.health <= 0) {
    ...
    return;
  } else if (partner.health === 1) {
    console.log("But looks ill :(");
  }
  // WE CAN ADD THINGS HERE TO MAKE IT INTERESTING!!
  ...
  inquirer
    .prompt([
      {
        type: "rawlist",
        name: "action",
        message: "What would you like to do with " + `${partner.name}` + "?",
    ...
    .then((answers) => {
      if (answers.action === "Feed") {
        ...
        partner.hydrate;
        console.log(`${partner.name}` + " " + "is drinking!");
